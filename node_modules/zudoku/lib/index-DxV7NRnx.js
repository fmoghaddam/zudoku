...
class Fe extends pt {
  ...
  toJSON(e, t) {
    ...
    let l = n.get(o);
    if (l || (j(o, null, t), l = n.get(o)), l?.res === void 0) {
...
` && k !== "	" && (u = h);
  ...
  for (let S = 0; S < c.length; ++S) {
    ...
    k === 0 ? w = `
${e}${s.slice(0, O)}` : (t === Be && d[k] && (w += `${s[k]}\\`), w += `
${e}${s.slice(k + 1, O)}`);
  }
  return w;
}
...
function Me({ comment: s, type: e, value: t }, n, i, r) {
  ...
  if (!o || /\n[\t ]+$/.test(t))
    return le(t, n);
  const c = n.indent || (n.forceBlockIndent || Je(t) ? "  " : ""), d = o === "literal" ? !0 : o === "folded" || e === N.BLOCK_FOLDED ? !1 : e === N.BLOCK_LITERAL ? !0 : !qs(t, a, c.length);
  if (!t)
    return d ? `|
` : `>
`;
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g, "$1$2").replace(/\n+/g, `$&${c}`);
    let A = !1;
    const I = Ve(n, !0);
    o !== "folded" && e !== N.BLOCK_FOLDED && (I.onOverflow = () => {
      A = !0;
    });
    const g = Ue(`${w}${O}${m}`, c, at, I);
    if (!A)
      return `>${k}
${c}${g}`;
  }
...
function Vs({ key: s, value: e }, t, n, i) {
  ...
  let m = !f && (!s || u && e == null && !t.inFlow || L(s) || (E(s) ? s.type === N.BLOCK_FOLDED || s.type === N.BLOCK_LITERAL : typeof s == "object"));
  t = Object.assign({}, t, {
    allNullValues: !1,
    implicitKey: !m && (f || !r),
    indent: l + a
...
${t.indent}`;
    ...
    const I = O[0], g = O.indexOf(`
`), _ = g !== -1, J = t.inFlow ?? e.flow ?? e.items.length === 0;
    if (_ || !J) {
      let te = !1;
      if (_ && (I === "&" || I === "!")) {
        let C = O.indexOf(" ");
        I === "&" && C !== -1 && C < g && O[C + 1] === "!" && (C = O.indexOf(" ", C + 1)), (C === -1 || g < C) && (te = !0);
      }
      te || (A = `
${t.indent}`);
    }
...
`) && (A = "");
  return p += A + O, t.inFlow ? k && n && n() : S && !k ? p += H(p, t.indent, c(S)) : h && i && i(), p;
}
function xt(s, e) {
...
const Te = "<<", V = {
  ...
  test: /^<<$/,
  resolve: () => Object.assign(new N(Symbol(Te)), {
    addToJSMap: Zt
  }),
  stringify: () => Te
}, Js = (s, e) => (V.identify(e) || E(e) && (!e.type || e.type === N.PLAIN) && V.identify(e.value)) && s?.doc.schema.tags.some((t) => t.tag === V.tag && t.default);
function Zt(s, e, t) {
...
function ue(s, { flow: e, indicator: t, next: n, offset: i, onError: r, parentIndent: o, startOnNewline: l }) {
  let a = !1, c = l, d = l, f = "", u = "", m = !1, y = !1, h = null, p = null, b = null, w = null, S = null, k = null, O = null;
  for (const g of s)
    switch (y && (g.type !== "space" && g.type !== "newline" && g.type !== "comma" && r(g.offset, "MISSING_CHAR", "Tags and anchors must be separated from the next token by white space"), y = !1), h && (c && g.type !== "comment" && g.type !== "newline" && r(h, "TAB_AS_INDENT", "Tabs are not allowed as indentation"), h = null), g.type) {
      ...
      case "newline":
        ...
        break;
      case "anchor":
        p && r(g, "MULTIPLE_ANCHORS", "A node can have at most one anchor"), g.source.endsWith(":") && r(g.offset + g.source.length - 1, "BAD_ALIAS", "Anchor ending in : is ambiguous", !0), p = g, O ?? (O = g.offset), c = !1, d = !1, y = !0;
        break;
      case "tag": {
        b && r(g, "MULTIPLE_TAGS", "A node can have at most one tag"), b = g, O ?? (O = g.offset), c = !1, d = !1, y = !0;
        break;
      }
      case t:
...
function un({ composeNode: s, composeEmptyNode: e }, t, n, i, r) {
  ...
  for (let p = 0; p < n.items.length; ++p) {
    const b = n.items[p], { start: w, key: S, sep: k, value: O } = b, A = ue(w, {
      flow: l,
      indicator: "explicit-key-ind",
      next: S ?? k?.[0],
      offset: f,
...
` + I : g.comment = I, A.comment = A.comment.substring(I.length + 1);
    ...
    if (!o && !k && !A.found) {
      const I = O ? s(t, O, A, i) : e(t, A.end, k, null, A, i);
      c.items.push(I), f = I.range[2], nt(O) && i(I.range, "BLOCK_IN_FLOW", st);
    } else {
      t.atKey = !0;
      ...
      } else O && ("source" in O && O.source?.[0] === ":" ? i(O, "MISSING_CHAR", `Missing space after : in ${l}`) : i(_.start, "MISSING_CHAR", `Missing , or : between ${l} items`));
      const J = O ? s(t, O, _, i) : _.found ? e(t, _.end, k, null, _, i) : null;
      J ? nt(O) && i(J.range, "BLOCK_IN_FLOW", st) : _.comment && (g.comment ? g.comment += `
...
function hn(s, e, t, n, i) {
  ...
  const c = it(s, e, t, i, o, a), d = a.resolve?.(c, (u) => i(r, "TAG_RESOLVE_FAILED", u), e.options) ?? c, f = $(d) ? d : new N(d);
...
function ws(s, e, t) {
  ...
  const r = i.mode === ">" ? N.BLOCK_FOLDED : N.BLOCK_LITERAL, o = e.source ? pn(e.source) : [];
  let l = o.length;
...
//# sourceMappingURL=index-DxV7NRnx.js.map
