...
const createTestContext = () => {
    ...
    const wrapper = ({ children }) => (_jsx(QueryClientProvider, { client: queryClient, children: _jsx(ZudokuProvider, { context: context, children: children }) }));
...
describe("useEvent", () => {
    ...
    it("transforms event data with callback", async () => {
        const { context, wrapper } = createTestContext();
        const { result } = renderHook(() => useEvent("location", ({ to }) => to.pathname), { wrapper });
        await act(() => Promise.resolve());
        await act(async () => {
            context.emitEvent("location", { to: locationData });
        });
    ...
    it("handles side effects without return value", async () => {
        ...
        const { result } = renderHook(() => useEvent("location", (event) => {
            sideEffect(event);
        }), { wrapper });
        await act(() => Promise.resolve());
        await act(async () => {
    ...
    it("removes event listener on cleanup", async () => {
        const { context, wrapper } = createTestContext();
        const sideEffect = vi.fn();
        const { unmount } = renderHook(() => useEvent("location", (event) => {
            sideEffect(event);
        }), { wrapper });
        await act(() => Promise.resolve());
    ...
    describe("types", () => {
        ...
        it("infers event type when no callback is provided", async () => {
            await act(async () => {
                ...
                assertType(hook.result.current);
            });
        });
        it("infers string type from pathname callback", () => {
            const hook = renderHook(() => useEvent("location", ({ to }) => to.pathname), { wrapper });
            assertType(hook.result.current);
        });
        it("infers object type from object callback", () => {
            const hook = renderHook(() => useEvent("location", ({ to }) => ({ query: to.search })), { wrapper });
            assertType(hook.result.current);
        });
...
//# sourceMappingURL=useEvent.test.js.map