...
export class OpenIDAuthenticationProvider extends CoreAuthenticationPlugin {
    client;
    issuer;
    authorizationServer;
    callbackUrlPath;
    ...
    audience;
    scopes;
    constructor({ issuer, audience, clientId, redirectToAfterSignUp, redirectToAfterSignIn, redirectToAfterSignOut = "/", basePath, scopes, }) {
        super();
        this.client = {
            client_id: clientId,
    ...
    async signUp(_, { redirectTo, replace = false, } = {}) {
    ...
    async signIn(_, { redirectTo, replace = false }) {
        return this.authorize({
    ...
    async authorize({ redirectTo, isSignUp = false, replace = false, }) {
    ...
    async getAccessToken() {
        ...
        if (new Date(tokenState.expiresOn) < new Date()) {
            ...
            const response = await oauth.refreshTokenGrantRequest(as, this.client, oauth.None(), tokenState.refreshToken);
    ...
    handleCallback = async () => {
        ...
        redirectUrl.search = "";
        redirectUrl.hash = "";
        const response = await oauth.authorizationCodeGrantRequest(authServer, this.client, oauth.None(), params, redirectUrl.toString(), codeVerifier);
        const oauthResult = await oauth.processAuthorizationCodeResponse(authServer, this.client, response);
