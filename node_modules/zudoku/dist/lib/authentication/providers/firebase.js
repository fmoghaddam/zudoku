...
import { createUserWithEmailAndPassword, getAuth, sendEmailVerification, sendPasswordResetEmail, signInWithEmailAndPassword, signInWithPopup, signOut, } from "firebase/auth";
...
import { ZudokuPasswordResetUi, ZudokuSignInUi, ZudokuSignUpUi, } from "../ui/ZudokuAuthUi.js";
class FirebaseAuthenticationProvider extends CoreAuthenticationPlugin {
    app;
    auth;
    providers;
    ...
    signUp = async ({ navigate }, { redirectTo }) => {
        void navigate(redirectTo
            ? `/signup?redirectTo=${encodeURIComponent(redirectTo)}`
            : `/signup`);
    };
    signIn = async ({ navigate }, { redirectTo }) => {
        void navigate(redirectTo
            ? `/signin?redirectTo=${encodeURIComponent(redirectTo)}`
            : `/signin`);
    };
    requestEmailVerification = async ({ navigate }, { redirectTo }) => {
    ...
    getRoutes = () => {
        return [
            {
                path: "/verify-email",
                element: (_jsx(EmailVerificationUi, { onResendVerification: async () => {
                        if (!this.auth.currentUser) {
                            throw new ZudokuError("User is not authenticated", {
                                title: "User not authenticated",
                            });
            ...
            {
                path: "/reset-password",
                element: (_jsx(ZudokuPasswordResetUi, { onPasswordReset: async (email) => {
                        try {
            ...
            {
                path: "/signin",
                element: (_jsx(ZudokuSignInUi, { providers: this.providers, enableUsernamePassword: this.enableUsernamePassword, onOAuthSignIn: async (providerId) => {
                    ...
                    }, onUsernamePasswordSignIn: async (email, password) => {
                        try {
                            useAuthState.setState({ isPending: false });
            ...
            {
                path: "/signup",
                element: (_jsx(ZudokuSignUpUi, { providers: this.providers, enableUsernamePassword: this.enableUsernamePassword, onOAuthSignUp: async (providerId) => {
                        const provider = await getProviderForId(providerId);
                        if (!provider) {
                            throw new AuthorizationError(`Provider ${providerId} not found`);
                        }
                        await signInWithPopup(this.auth, provider);
                    }, onUsernamePasswordSignUp: async (email, password) => {
                        useAuthState.setState({ isPending: true });
...
const supabaseAuth = (options) => new FirebaseAuthenticationProvider(options);
export default supabaseAuth;
const getProviderForId = async (providerId) => {
    switch (providerId) {
        case "google": {
