...
const createZuploService = ({ deploymentName, ...options }) => {
    return {
        ...
        updateConsumer: async (consumer, context) => {
            invariant(deploymentName, "Cannot update API key description.", developerHintOptions);
            const response = await fetch(await context.signRequest(new Request(`${DEFAULT_API_KEY_ENDPOINT}/${deploymentName}/consumers/${consumer.id}`, {
                method: "PATCH",
                headers: {
        ...
        rollKey: async (consumerId, context) => {
            ...
            const response = await fetch(await context.signRequest(new Request(`${DEFAULT_API_KEY_ENDPOINT}/${deploymentName}/consumers/${consumerId}/roll-key`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
...
export const apiKeyPlugin = ({ deploymentName, isZuplo, ...options }) => {
    if (isZuplo && !deploymentName) {
        // biome-ignore lint/suspicious/noConsole: Important warning
        console.warn("This project is not linked to a Zuplo deployment. Run `zuplo link` to get started.");
    }
    ...
    return {
        ...
        getRoutes: () => {
            return [
                {
                    path: "/settings/api-keys",
                    element: _jsx(SettingsApiKeys, { service: verifiedService }),
                },
            ];
        },
        getProtectedRoutes: () => {
