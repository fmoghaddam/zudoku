...
export const openApiPlugin = (config) => {
    ...
    return {
        getHead: () => {
            if (config.type === "url" && !config.skipPreload) {
                const urls = Array.isArray(config.input)
                    ...
                    : [config.input];
                return urls.map((url) => (_jsx("link", { href: url, rel: "preload", as: "fetch", crossOrigin: "anonymous" }, url)));
            }
            if (config.server) {
        ...
        getMdxComponents: () => ({
            OpenPlaygroundButton: ({ server, method = "get", url = "/", children, ...props }) => {
                if (!server) {
                    throw new Error("Server is required");
                }
                return (_jsx(PlaygroundDialog, { url: url, method: method, server: server, ...props, children: _jsx(Button, { className: "gap-2 items-center", variant: "outline", children: children ?? (_jsxs(_Fragment, { children: ["Open in Playground", _jsx(CirclePlayIcon, { size: 16 })] })) }) }));
            },
        }),
