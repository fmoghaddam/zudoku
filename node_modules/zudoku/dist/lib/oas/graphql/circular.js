...
obj, currentPath = new WeakSet(), refs = new WeakMap(), path = [], currentRefPaths = new Set()) => {
    if (obj === null || typeof obj !== "object")
        return obj;
    const refPath = obj.__$ref;
    const isCircular = currentPath.has(obj) ||
    ...
    currentPath.add(obj);
    if (typeof refPath === "string")
        currentRefPaths.add(refPath);
    const recurse = (value, key) => handleCircularRefs(value, currentPath, refs, [...path, key], currentRefPaths);
    const result = Array.isArray(obj)
        ? obj.map((item, i) => recurse(item, i.toString()))
        : Object.fromEntries(Object.entries(obj).map(([k, v]) => [k, recurse(v, k)]));
    refs.set(obj, result);
    currentPath.delete(obj);
    if (typeof refPath === "string")
        currentRefPaths.delete(refPath);
    return result;
};
export const GraphQLJSONSchema = new GraphQLScalarType({
    ...
    serialize: (value) => GraphQLJSON.serialize(handleCircularRefs(value)),
});
