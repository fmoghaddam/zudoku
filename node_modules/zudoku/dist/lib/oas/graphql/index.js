...
export const getAllTags = (schema, slugs) => {
    ...
    const tagGroups = schema["x-tagGroups"] ?? [];
    if (tagGroups.length === 0)
        return result;
    const groupOrder = tagGroups.flatMap((group) => group.tags);
    return result.sort((a, b) => {
        if (!a.name || !b.name)
            return 0; // Keep untagged at end
        const indexA = groupOrder.indexOf(a.name);
        const indexB = groupOrder.indexOf(b.name);
        if (indexA === -1 && indexB === -1)
            return 0; // Keep original order for ungrouped
        if (indexA === -1)
            return 1; // Ungrouped after grouped
        if (indexB === -1)
            return -1; // Grouped before ungrouped
...
export const getAllOperations = (paths) => {
    const operations = Object.entries(paths ?? {}).flatMap(([path, value]) => HttpMethods.flatMap((method) => {
        ...
        const servers = operationServers.length > 0 ? operationServers : pathServers;
        return {
            ...operation,
            method,
            path,
        ...
        responses: t.field({
            ...
            resolve: (parent) => {
                return Object.entries(parent.responses ?? {}).map(([statusCode, response]) => ({
                    ...
                    content: Object.entries(response.content ?? {}).map(([mediaType, { schema, examples, example }]) => ({
                        mediaType,
                        schema,
                        examples: examples
