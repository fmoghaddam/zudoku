...
export const flattenAllOf = (schema) => {
    ...
    if (items != null && typeof items === "object") {
        if (Array.isArray(items)) {
            merged.items = items.map((it) => it && typeof it === "object" ? flattenAllOf(it) : it);
        }
        else {
    ...
    for (const key of ["anyOf", "oneOf"]) {
        ...
        if (!Array.isArray(arr))
            continue;
        merged[key] = arr.map((v) => typeof v === "object" && v != null ? flattenAllOf(v) : v);
    }
    return merged;
};
//# sourceMappingURL=flattenAllOf.js.map