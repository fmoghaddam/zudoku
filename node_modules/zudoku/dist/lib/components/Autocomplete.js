...
const AutocompletePopover = ({ value, options, onChange, className, placeholder = "Value", onEnterPress, onKeyDown, ref, onSelect, }) => {
    const [open, setOpen] = useState(false);
    const [dontClose, setDontClose] = useState(false);
    const count = useCommandState((state) => state.filtered.count);
    const inputRef = useRef(null);
    return (_jsxs(Popover, { open: open, children: [_jsx(PopoverAnchor, { children: _jsx(CommandInlineInput, { autoComplete: "off", ref: (el) => {
                        inputRef.current = el;
                    ...
                    }, value: value ? String(value) : undefined, placeholder: placeholder, className: cn("h-9 bg-transparent", className), onFocus: () => setOpen(true), onBlur: () => {
                        if (dontClose) {
                            return;
                        }
                        setOpen(false);
