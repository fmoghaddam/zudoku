...
export function findOutputPathOfServerConfig(output) {
    ...
    if ("output" in output) {
        const result = output.output.find((o) => "isEntry" in o && o.isEntry && o.fileName === "zudoku.config.js");
        if (result) {
...
function loadEnv(configEnv, rootDir) {
...
async function updateModifiedTimes() {
    ...
    await Promise.all(files.map(async (depPath) => {
        const depStat = await stat(depPath);
...
export const setCurrentConfig = (newConfig) => {
    config = newConfig;
};
export async function loadZudokuConfig(configEnv, rootDir) {
...
export const setStandaloneConfig = (rootDir) => {
    config = {
        __meta: {
            rootDir,
            moduleDir: getModuleDir(),
