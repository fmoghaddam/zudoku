...
export const globMarkdownFiles = async (config, options = { absolute: false }) => {
 ...
 */
export const resolveCustomNavigationPaths = async (config, fileMapping) => {
...
const viteDocsPlugin = () => {
    ...
    return {
        ...
        async load(id) {
            ...
            const globImportBasePath = process.env.NODE_ENV === "development" ? (config.basePath ?? "") : "";
            // Glob markdown files and resolve custom navigation paths
            const fileMapping = await resolveCustomNavigationPaths(config, await globMarkdownFiles(config, { absolute: false }));
            // Transform file paths to import paths
            const globbedDocuments = {};
            for (const [routePath, file] of Object.entries(fileMapping)) {
                const importPath = ensureLeadingSlash(path.posix.join(globImportBasePath, file));
                globbedDocuments[routePath] = importPath;
            }
            code.push(`const fileImports = {`, ...Object.entries(globbedDocuments).map(([routePath, importPath]) => `  "${routePath}": () => import("${importPath}"),`), `};`, `export const configuredDocsPlugin = markdownPlugin({`, `  basePath: "${config.basePath ?? ""}",`, `  fileImports,`, `  defaultOptions: ${JSON.stringify(docsConfig.defaultOptions)},`, `});`);
