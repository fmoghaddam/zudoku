...
const rehypeNormalizeMdxImages = () => (tree) => {
    visit(tree, ["mdxJsxFlowElement", "mdxJsxElement"], (node) => {
        if (node.type !== "mdxJsxFlowElement")
            return;
        if (!["img", "video"].includes(node.name))
            return;
...
const rehypeExcerptWithMdxExport = () => (tree) => {
    let excerpt;
    visit(tree, "element", (node, _index, parent) => {
        if (node.tagName !== "p" || parent?.type !== "root")
            return;
...
const viteMdxPlugin = async () => {
    ...
    const remarkPlugins = typeof buildConfig?.remarkPlugins === "function"
    ...
    const rehypePlugins = typeof buildConfig?.rehypePlugins === "function"
        ? buildConfig.rehypePlugins(defaultRehypePlugins)
        : [...defaultRehypePlugins, ...(buildConfig?.rehypePlugins ?? [])];
    return {
        enforce: "pre",
        ...mdx({
            providerImportSource: config.__meta.mode === "internal" || config.__meta.mode === "standalone"
                ? "@mdx-js/react"
                : "zudoku/components",
            mdxExtensions: [".mdx"],
            format: "detect",
