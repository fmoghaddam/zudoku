...
export async function generateLlmsTxtFiles({ markdownFileInfos, outputUrls, basePath, baseOutputDir, siteName, llmsTxt, llmsTxtFull, workerResults, }) {
    // Filter out redirects
    const redirectUrls = new Set(workerResults
    ...
    const markdownMap = new Map(markdownFileInfos.map((info) => [info.routePath, info]));
    // Generate llms.txt
    if (llmsTxt) {
        const llmsTxtParts = [];
    ...
    if (llmsTxtFull) {
        ...
        await writeFile(path.join(baseOutputDir, "llms-full.txt"), llmsFull, "utf-8");
