...
const MEDIA_REGEX = /\.(a?png|jpe?g|gif|bmp|svg|webp|tiff|ico|webm|ogg|mp3|wav|m4a|avif|mp4)/i;
const defineEnvVars = (vars) => Object.fromEntries(vars.flatMap((v) => [
    [`process.env.${v}`, JSON.stringify(process.env[v])],
    [`import.meta.env.${v}`, JSON.stringify(process.env[v])],
]));
export async function getViteConfig(dir, configEnv) {
    const { config, publicEnv, envPrefix } = await loadZudokuConfig(configEnv, dir);
    ...
    const zudokuIncludeOptimizedDeps = [
        ...
        ...(config.syntaxHighlighting?.languages ?? defaultLanguages).map((lang) => `@shikijs/langs/${lang}`),
        ...Object.values(config.syntaxHighlighting?.themes ?? defaultHighlightOptions.themes).map((theme) => `@shikijs/themes/${theme}`),
    ].map((dep) => `zudoku > ${dep}`);
    // We define public env vars as `process.env` vars because Vite only exposes them as `import.meta.env` vars
    const publicVarsProcessEnvDefine = Object.fromEntries(Object.entries(publicEnv).map(([key, value]) => [
        `process.env.${key}`,
        value,
    ]));
    if (ZuploEnv.isZuplo) {
    ...
    const viteConfig = {
        ...
        build: {
            ...
            sourcemap: true,
            target: "es2022",
            outDir: path.resolve(path.join(dir, "dist", config.basePath ?? "", configEnv.isSsrBuild ? "server" : "")),
            emptyOutDir: true,
            rollupOptions: {
    ...
    const userConfig = await loadConfigFromFile(configEnv, undefined, dir, undefined, undefined, "runner");
    if (userConfig) {
        const merged = mergeConfig(viteConfig, userConfig.config, true);
