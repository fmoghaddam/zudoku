...
const renderPage = async ({ urlPath }) => {
    ...
    const isProtectedRoute = protectedRoutes
        ? Object.keys(protectedRoutes).some((route) => matchPath({ path: route, end: true }, urlPath))
        : false;
    let html;
    ...
    if (isProtectedRoute) {
        ...
        if (bypassResponse.statusCode >= 500) {
            throw new Error(`SSR failed (bypass render) with status ${bypassResponse.statusCode} for path: ${urlPath}`);
        }
