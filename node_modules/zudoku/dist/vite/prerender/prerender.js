...
export const prerender = async ({ html, dir, basePath = "", serverConfigFilename, writeRedirects = true, }) => {
    ...
    if (!isTTY()) {
        logger.info(colors.dim(`prerendering ${paths.length} routes using ${maxThreads} workers...`));
    }
    ...
    const workerResults = await Promise.all(paths.map(async (urlPath) => {
        ...
        if (isTTY()) {
            ...
            if (now - lastLogTime >= LOG_INTERVAL_MS) {
                logger.info(colors.blue(`prerendered ${completedCount}/${paths.length} routes using ${maxThreads} workers`));
                lastLogTime = now;
            }
        }
        return result;
    ...
    if (config.docs) {
        const { DocsConfigSchema } = await import("../../config/validators/validate.js");
        ...
        const markdownInfoPath = path.join(dir, "node_modules/.zudoku/markdown-info.json");
        ...
        if (!docsConfig.publishMarkdown) {
            await Promise.all(markdownFileInfos.map((info) => rm(info.filePath).catch(() => { })));
        }
    }
