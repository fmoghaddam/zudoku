...
import { getAllOperations, getAllSlugs, getAllTags, } from "../lib/oas/graphql/index.js";
...
const viteApiPlugin = async () => {
    ...
    const zuploProcessors = ZuploEnv.isZuplo
        ? await runnerImport(path.resolve(import.meta.dirname, "../zuplo/with-zuplo-processors.js")).then((m) => m.module.default(initialConfig.__meta.rootDir))
        : [];
    const buildConfig = await getBuildConfig();
    const buildProcessors = buildConfig?.processors ?? [];
    const tmpStoreDir = path.posix.join(initialConfig.__meta.rootDir, "node_modules/.zudoku/processed");
    ...
    return {
        ...
        configureServer(server) {
            ...
            server.middlewares.use(async (req, res, next) => {
                if (req.method !== "GET" || !req.url)
                    return next();
                if (!req.url.toLowerCase().endsWith(".json") &&
                ...
                const mimeType = path.extname(inputPath).toLowerCase() === ".json"
                    ? "application/json"
                    : "application/x-yaml";
        ...
        async load(id) {
            ...
            if (config.apis) {
                ...
                code.push(`const apis = Array.isArray(config.apis) ? config.apis : [config.apis]`);
                ...
                for (const apiConfig of apis) {
                    ...
                    if (apiConfig.type === "file") {
                        ...
                        if (!schemas?.length)
                            continue;
                        const tags = Array.from(new Set(schemas
                            .flatMap(({ schema }) => {
                            ...
                            .flatMap(({ slug }) => slug ?? [])));
                        const schemaMapEntries = Array.from(schemaManager.schemaMap.entries());
                        ...
                        code.push("configuredApiPlugins.push(openApiPlugin({", `  type: "file",`, `  input: ${JSON.stringify(versionedInput)},`, `  path: ${JSON.stringify(apiConfig.path)},`, `  tagPages: ${JSON.stringify(tags)},`, `  options: {`, `    examplesLanguage: config.defaults?.apis?.examplesLanguage ?? config.defaults?.examplesLanguage,`, `    supportedLanguages: config.defaults?.apis?.supportedLanguages,`, `    disablePlayground: config.defaults?.apis?.disablePlayground,`, `    disableSidecar: config.defaults?.apis?.disableSidecar,`, `    showVersionSelect: config.defaults?.apis?.showVersionSelect ?? "if-available",`, `    expandAllTags: config.defaults?.apis?.expandAllTags ?? true,`, `    expandApiInformation: config.defaults?.apis?.expandApiInformation ?? false,`, `    schemaDownload: config.defaults?.apis?.schemaDownload,`, `    transformExamples: config.defaults?.apis?.transformExamples,`, `    generateCodeSnippet: config.defaults?.apis?.generateCodeSnippet,`, `    ...(apis[${apiIndex}].options ?? {}),`, `  },`, `  schemaImports: {`, ...schemaMapEntries.map(([key, processed]) => `    "${key.replace(/\\/g, "\\\\")}": () => import("${processed.filePath.replace(/\\/g, "/")}?d=${processed.processedTime}"),`), `  },`, "}));");
                    }
                    else {
                        code.push("configuredApiPlugins.push(openApiPlugin({", `  ...${JSON.stringify(apiConfig)},`, "  options: {", `    examplesLanguage: config.defaults?.apis?.examplesLanguage ?? config.defaults?.examplesLanguage,`, `    supportedLanguages: config.defaults?.apis?.supportedLanguages,`, `    disablePlayground: config.defaults?.apis?.disablePlayground,`, `    disableSidecar: config.defaults?.apis?.disableSidecar,`, `    showVersionSelect: config.defaults?.apis?.showVersionSelect ?? "if-available",`, `    expandAllTags: config.defaults?.apis?.expandAllTags ?? false,`, `    schemaDownload: config.defaults?.apis?.schemaDownload,`, `    ...${JSON.stringify(apiConfig.options ?? {})},`, "  },", "}));");
                    }
                }
                if (config.catalogs) {
                    code.push('import { apiCatalogPlugin } from "zudoku/plugins/api-catalog";');
                    ...
                    const categoryList = Array.from(categories.entries()).map(([label, tags]) => ({
                        label,
                    ...
                    for (let i = 0; i < catalogs.length; i++) {
                        ...
                        code.push(`configuredApiCatalogPlugins.push(apiCatalogPlugin({`, `  ...${JSON.stringify(apiCatalogConfig, null, 2)},`, `  filterCatalogItems: Array.isArray(config.catalogs)`, `    ? config.catalogs[${i}].filterItems`, `    : config.catalogs.filterItems,`, `}));`);
                    }
                }
            }
