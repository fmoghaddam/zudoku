...
import { fetchShadcnRegistryItem, } from "./shadcn-registry.js";
...
const generateCss = (theme) => objectEntries(theme)
    ...
    .join("\n");
const generateRegistryCss = (cssVars) => Object.entries(cssVars)
    .map(([key, value]) => `  --${key}: ${value};`)
    .join("\n");
const processCustomCss = (css) => {
    if (typeof css === "string") {
        return css;
    }
    const processStyles = (styles, indent = "") => {
...
const getGoogleFontUrl = (font) => `https://fonts.googleapis.com/css2?family=${font.replaceAll(/\s+/g, "+")}:wght@400;500;600;700&display=swap`;
const processFont = (fontValue) => {
    const families = fontValue
        .split(",")
        .map((f) => f.trim().replaceAll(/['"]/g, ""));
    return families
        .filter((family) => GOOGLE_FONTS.includes(family))
        .map((family) => getGoogleFontUrl(family));
};
const processFontConfig = (fontConfig) => {
    if (!fontConfig)
        return { url: undefined, fontFamily: undefined };
    if (typeof fontConfig === "string") {
        if (GOOGLE_FONTS.includes(fontConfig)) {
            return {
...
const processFonts = async (themeConfig) => {
    const imports = [];
    const families = {
    ...
    if (themeConfig?.registryUrl) {
        try {
            const registryItem = await fetchShadcnRegistryItem(themeConfig.registryUrl);
...
export const viteThemePlugin = () => {
    return {
        ...
        async load(id) {
            ...
            if (themeConfig.registryUrl) {
                try {
                    ...
                    const applyOverrides = (vars, overrides) => {
                        if (!overrides)
                            return vars;
                        const result = { ...vars };
                    ...
                    if (lightVars["letter-spacing"] !== "0em") {
                        themeCss.push("@theme inline {", "  --tracking-tighter: calc(var(--tracking-normal) - 0.05em);", "  --tracking-tight: calc(var(--tracking-normal) - 0.025em);", "  --tracking-normal: var(--tracking-normal);", "  --tracking-wide: calc(var(--tracking-normal) + 0.025em);", "  --tracking-wider: calc(var(--tracking-normal) + 0.05em);", "  --tracking-widest: calc(var(--tracking-normal) + 0.1em);", "}");
            ...
            rootVars.push(`  --shiki-light: ${lightTheme?.fg ?? "#000"};`, `  --shiki-dark: ${darkTheme?.fg ?? "#fff"};`, `  --shiki-light-bg: ${lightTheme?.bg ?? "#fff"};`, `  --shiki-dark-bg: ${darkTheme?.bg ?? "#000"};`);
            themeCss.push(`:root {\n${rootVars.join("\n")}\n}`);
        ...
        async transform(src, id) {
            ...
            const tailwindSources = (config.__tailwindSources ?? []).map((source) => path.relative(path.dirname(id), source));
            const code = [...files, ...tailwindSources].map((file) => `@source "${file}";`);
            ...
            code.push(...colorVars.map((color) => `  --color-${color}: var(--${color});`));
            code.push(`  --radius-sm: calc(var(--radius) - 4px);`, `  --radius-md: calc(var(--radius) - 2px);`, `  --radius-lg: var(--radius);`, `  --radius-xl: calc(var(--radius) + 4px);`, `  --font-sans: var(--font-sans);`, `  --font-mono: var(--font-mono);`, `  --font-serif: var(--font-serif);`);
            code.push("}");
...
//# sourceMappingURL=plugin-theme.js.map