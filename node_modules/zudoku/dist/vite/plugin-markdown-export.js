...
import { globMarkdownFiles, resolveCustomNavigationPaths, } from "./plugin-docs.js";
 ...
 */
const viteMarkdownExportPlugin = () => {
    let markdownFiles = {};
    ...
    return {
        ...
        async buildStart() {
            ...
            const needsMdFiles = config.docs?.publishMarkdown ||
                llmsConfig?.llmsTxt ||
                llmsConfig?.llmsTxtFull;
            if (config.__meta.mode === "standalone" || !needsMdFiles) {
                return;
            }
            markdownFiles = await resolveCustomNavigationPaths(config, await globMarkdownFiles(config, { absolute: true }));
            ...
            if (!llmsConfig?.includeProtected) {
                ...
                if (protectedRoutes) {
                    const isProtectedRoute = (routePath) => {
                        return Object.keys(protectedRoutes).some((route) => matchPath({ path: route, end: true }, routePath));
                    };
        ...
        async configureServer(server) {
            ...
            server.middlewares.use(async (req, res, next) => {
                ...
                const routePath = joinUrl(req.url.slice(basePath.length).replace(/\.mdx?$/, ""));
        ...
        async closeBundle() {
            ...
            const distDir = path.join(config.__meta.rootDir, "dist", config.basePath ?? "");
            markdownFileInfos = [];
            for (const [routePath, filePath] of Object.entries(markdownFiles)) {
                try {
                    const { content: finalMarkdown, title, description, } = await processMarkdownFile(filePath);
            ...
            if (config.docs?.llms?.llmsTxt || config.docs?.llms?.llmsTxtFull) {
                const markdownInfoPath = path.join(config.__meta.rootDir, "node_modules/.zudoku/markdown-info.json");
                await writeFile(markdownInfoPath, JSON.stringify(markdownFileInfos, null, 2), "utf-8");
            }
        },
    };
};
export default viteMarkdownExportPlugin;
//# sourceMappingURL=plugin-markdown-export.js.map