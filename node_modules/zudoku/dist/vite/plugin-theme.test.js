...
const callPluginLoad = async (plugin, id) => {
    // biome-ignore lint/style/noNonNullAssertion: is guaranteed to be defined
    const hook = plugin.load;
    const loadFn = typeof hook === "function" ? hook : hook.handler;
    return loadFn.call({}, id);
};
const callPluginTransform = async (plugin, src, id) => {
    // biome-ignore lint/style/noNonNullAssertion: is guaranteed to be defined
    const hook = plugin.transform;
    const transformFn = typeof hook === "function" ? hook : hook.handler;
    return transformFn.call({}, src, id);
};
describe("plugin-theme", () => {
    ...
    describe("fonts", () => {
        it("should handle predefined Google fonts", async () => {
            ...
            expect(result).toContain("@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap')");
            expect(result).toContain("@import url('https://fonts.googleapis.com/css2?family=Merriweather:wght@400;500;600;700&display=swap')");
            expect(result).toContain("@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&display=swap')");
        ...
        it("should handle custom font objects", async () => {
            ...
            const plugin = viteThemePlugin();
            const result = await callPluginLoad(plugin, resolvedVirtualModuleId);
            expect(result).toContain("@import url('http://example.com/custom.woff2')");
        ...
        it("should use default fonts when no fonts are configured", async () => {
            ...
            const plugin = viteThemePlugin();
            const result = await callPluginLoad(plugin, resolvedVirtualModuleId);
            expect(result).toContain("@import url('https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&display=swap')");
            expect(result).toContain("@import url('https://fonts.googleapis.com/css2?family=Geist+Mono:wght@400;500;600;700&display=swap')");
    ...
    describe("custom CSS", () => {
        it("should handle custom CSS as string", async () => {
            ...
            const plugin = viteThemePlugin();
            const result = await callPluginTransform(plugin, "/* @vite-plugin-inject main */", "/test/src/app/main.css");
    ...
    describe("transform function", () => {
        ...
        it("should not transform non-main.css files", async () => {
            const plugin = viteThemePlugin();
            const result = await callPluginTransform(plugin, "/* @vite-plugin-inject main */", "/test/src/other.css");
    ...
    describe("registry integration", () => {
        it("should handle registry errors gracefully", async () => {
            ...
            vi.mocked(fetchShadcnRegistryItem).mockRejectedValue(new Error("Network error"));
            const consoleSpy = vi
                .spyOn(console, "error")
                .mockImplementation(() => { });
            const plugin = viteThemePlugin();
            const result = await callPluginLoad(plugin, resolvedVirtualModuleId);
            expect(consoleSpy).toHaveBeenCalledWith("Failed to load shadcn registry theme:", expect.any(Error));
        ...
        it("should handle successful registry calls", async () => {
            ...
            const transformResult = await callPluginTransform(plugin, "/* @vite-plugin-inject main */", "/test/src/app/main.css");
            expect(transformResult).toContain(".base {");
            expect(transformResult).toContain("--root: 1;");
        });
    });
});
//# sourceMappingURL=plugin-theme.test.js.map