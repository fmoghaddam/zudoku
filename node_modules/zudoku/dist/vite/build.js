...
import { findOutputPathOfServerConfig, loadZudokuConfig, } from "../config/loader.js";
...
export async function runBuild(options) {
    try {
        ...
        const { config } = await loadZudokuConfig({ mode: "production", command: "build" }, options.dir);
        ...
        if ("output" in clientResult) {
            ...
            try {
                ...
                const statusPages = results.flatMap((r) => /400|404|500\.html$/.test(r.outputPath) ? r.outputPath : []);
                // move status pages to root path (i.e. without base path)
                for (const statusPage of statusPages) {
                    await rename(statusPage, path.join(options.dir, DIST_DIR, path.basename(statusPage)));
                }
                ...
                if (process.env.VERCEL) {
                    ...
                    await rename(path.join(options.dir, DIST_DIR), path.join(options.dir, ".vercel/output/static"));
                }
                ...
                if (ZuploEnv.isZuplo && issuer) {
                    await writeFile(path.join(options.dir, DIST_DIR, ".output/zuplo.json"), JSON.stringify({ issuer }, null, 2), "utf-8");
                }
            }
            catch (e) {
