...
const warnShallowClone = () => {
    if (hasWarnedShallowClone)
        return;
    hasWarnedShallowClone = true;
    if (process.env.VERCEL) {
        console.warn("The repository is shallow cloned, so the latest modified time may not be accurate. Set the VERCEL_DEEP_CLONE=true environment variable to enable deep cloning.");
    }
    else if (process.env.GITHUB_ACTIONS) {
        console.warn("The repository is shallow cloned, so the latest modified time may not be accurate. See https://github.com/actions/checkout#fetch-all-history-for-all-tags-and-branches to fetch all the history.");
    }
    else {
        console.warn("The repository is shallow cloned, so the latest modified time may not be accurate.");
    }
};
const getLastModifiedDate = async (filePath) => {
    if (checkGitAvailable()) {
        const result = spawnSync("git", ["log", "-1", "--format=%cI", "--", filePath], { encoding: "utf-8", stdio: ["pipe", "pipe", "ignore"] });
...
//# sourceMappingURL=remark-last-modified.js.map