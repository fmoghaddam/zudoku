...
export const warnPackageVersionMismatch = async () => {
    ...
    const packageJson = getPackageJson(fileURLToPath(new URL("../../../package.json", import.meta.url)));
    const nodeModulesPath = fileURLToPath(new URL("../../../..", import.meta.url));
    ...
    if (!installed.react ||
        ...
        !required.react ||
        !required["react-dom"]) {
        printWarningToConsole("Could not verify React version. Ensure react and react-dom are installed.");
        return;
    }
    if (satisfies(installed.react, required.react) &&
