...
import { createStaticHandler, createStaticRouter, isRouteErrorResponse, } from "react-router";
...
export const render = async ({ template, request: baseRequest, response, routes, basePath, bypassProtection, }) => {
    ...
    if (context instanceof Response) {
        if ([301, 302, 303, 307, 308].includes(context.status)) {
            return response.redirect(import.meta.env.PROD ? context.status : 307, context.headers.get("Location") ?? "");
        }
        throw context;
    }
    ...
    const helmetContext = {};
    const App = (_jsx(BootstrapStatic, { router: router, context: context, queryClient: queryClient, helmetContext: helmetContext, bypassProtection: bypassProtection }));
        ...
        onAllReady() {
            ...
            transformStream.on("finish", () => {
                const dehydrated = dehydrate(queryClient, {
                    shouldDehydrateQuery: (query) => !query.queryKey.includes(NO_DEHYDRATE),
                });
                if (!htmlEnd)
                    return response.end();
