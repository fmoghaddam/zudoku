...
if (process.env.SENTRY_DSN) {
    void import("./sentry.js").then((mod) => {
        mod.initSentry({ dsn: process.env.SENTRY_DSN });
    });
}
...
if (import.meta.env.IS_ZUPLO) {
    ...
    console.log(`%cPowered by Zuplo v${window.ZUDOKU_VERSION}`, [
        "color: #Df0097",
        "line-height: 50px",
        "font-weight: bolder",
        "font-size: 15px",
    ...
    // biome-ignore lint/suspicious/noConsole: Logging allowed here
    console.log(`%cZUDOKU%c by Zuplo v${window.ZUDOKU_VERSION}`, [
        "color: #FF00BD",
        "line-height: 50px",
        "font-weight: bolder",
        "font-size: 30px",
...
async function hydrateLazyRoutes(routes) {
    ...
    const lazyMatches = matchRoutes(routes, path, config.basePath)?.filter((m) => m.route.lazy);
