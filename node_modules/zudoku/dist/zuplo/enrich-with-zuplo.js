...
const enrichWithApiKeyData = (operationObject, apiKeyPolicies) => {
    if (apiKeyPolicies.length === 0) {
        return operationObject;
    }
    const firstPolicy = apiKeyPolicies[0];
    const authorizationHeader = firstPolicy?.handler.options?.authHeader || "Authorization";
    const authorizationScheme = (firstPolicy?.handler.options?.authScheme ??
        "Bearer");
    const authSchemeExample = authorizationScheme !== ""
    ...
    if (!parameters.some((param) => param.name === authorizationHeader)) {
    ...
    else if (!operationObject.security.some((req) => req[apiSecuritySchemeId])) {
...
const enrichWithRateLimitData = (operationObject, rateLimitPolicies) => {
    if (rateLimitPolicies.length === 0) {
        return operationObject;
    }
    const shouldIncludeHeader = rateLimitPolicies.some((policy) => policy.handler.options?.headerMode !== "none");
...
const rateLimitingResponseWithHeader = {
...
export const enrichWithZuploData = ({ policiesConfig, }) => {
    return ({ schema }) => {
        if (!schema.paths)
            return schema;
        let hasRateLimitPolicies = false;
        for (const [, pathItem] of objectEntries(schema.paths)) {
            for (const method of operations) {
                const operation = pathItem[method];
                if (!operation?.["x-zuplo-route"])
                    continue;
                const inboundPolicies = operation["x-zuplo-route"]?.policies?.inbound?.reduce((acc, policyName) => {
                    const policy = policiesConfig.policies?.find(({ name }) => name === policyName);
                    if (!policy)
                        return acc;
                    // Handle composite policies
                    if (policy.handler.export === "CompositeInboundPolicy") {
                        const childPolicies = policy.handler.options?.policies;
                ...
                const apiKeyPolicies = policiesConfig.policies?.filter((policy) => inboundPolicies.includes(policy.name) &&
                ...
                const rateLimitPolicies = policiesConfig.policies?.filter((policy) => inboundPolicies.includes(policy.name) &&
                ...
                pathItem[method] = enrichWithRateLimitData(pathItem[method], rateLimitPolicies);
            }
        }
        // Add security scheme if we have API key policies
        if (policiesConfig.policies?.some((policy) => policy.handler.export === "ApiAuthKeyInboundPolicy" ||
            policy.handler.export === "ApiKeyInboundPolicy")) {
...
//# sourceMappingURL=enrich-with-zuplo.js.map